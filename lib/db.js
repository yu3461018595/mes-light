...
function seed() {
  ...
  tx(() => {
    ...
    try {
      ...
      insert(`INSERT INTO incoming_materials(code,incoming_date,supplier,material_code,material_name,material_spec,qty,unit,batch,order_id,inspector,result,remark,created_by,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        ['LM26090101', '2026-09-01', '瑞泰机械有限公司', 'RM-001', '45#圆钢', 'Φ45', 500, 'kg', 'B250901', so ? so.id : null, '李伟', 'qualified', '首件检验合格', 1, now()]);
      insert(`INSERT INTO incoming_materials(code,incoming_date,supplier,material_code,material_name,material_spec,qty,unit,batch,order_id,inspector,result,remark,created_by,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        ['LM26090201', '2026-09-02', '华东汽车零部件厂', 'RM-002', '深沟球轴承', '6204-2RS', 200, '套', 'B250902', null, '陈静', 'pending', '待复检', 1, now()]);
      if (so && sp) {
        insert(`INSERT INTO finished_goods_in(code,in_date,order_id,product_code,product_name,spec,qty,unit,batch,location,inspector,result,remark,created_by,created_at)
          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          ['RK26090501', '2026-09-05', so.id, sp.code, sp.name, sp.spec, 300, '件', 'P250905', 'A-01-01', '王强', 'qualified', '完工入库', 1, now()]);
        insert(`INSERT INTO finished_goods_in(code,in_date,order_id,product_code,product_name,spec,qty,unit,batch,location,inspector,result,remark,created_by,created_at)
          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          ['RK26090601', '2026-09-06', null, 'P2002', '齿轮箱体', 'GB-220A', 120, '件', 'P250906', 'A-02-03', '刘洋', 'qualified', '补库入库', 1, now()]);
      }
